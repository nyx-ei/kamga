...
type MemberDashboardSection = 'applications' | 'contributions' | 'notifications' | 'payments' | 'receipts' | 'relatives';

type MemberDashboardContentProps = DashboardPageProps & {
  section: MemberDashboardSection;
};

export async function MemberDashboardContent({ params, searchParams, section }: MemberDashboardContentProps) {
  ...
  const notificationsT = await getTranslations('notifications');
    ...
    >
      <section className="flex flex-col gap-6">
        ...
        {section === 'relatives' ? (
          <section className="grid gap-4">
            <div className="space-y-2">
              <h2 className="text-xl font-semibold text-heading">{t('shares.title')}</h2>
              <p className="text-sm leading-6 text-secondary">{t('shares.description')}</p>
            </div>
            {applications.some((application) => application.status === 'active') ? (
              <div className="grid gap-4">
                {applications
                  ...
                  .map((application) => {
                    ...
                    return <DependentsManager dependents={dependents} key={application.id} membershipId={application.id} />;
                  })}
              </div>
            ) : (
              <div className="rounded-md border border-border bg-sunken p-6 text-sm leading-6 text-secondary">{t('shares.emptyState')}</div>
            )}
          </section>
        ) : null}

        {section === 'contributions' && joinRequests.length > 0 ? (
          <section className="grid gap-4">
            <div className="space-y-2">
        ...
        {section === 'contributions' && activeMemberContributions.length > 0 ? (
          <section className="grid gap-4">
            <div className="space-y-2">
        ...
        {section === 'contributions' && contributionHistory.length > 0 ? (
          <section className="grid gap-4">
            <div className="space-y-2">
        ...
        {section === 'contributions' && associationCalls.length > 0 ? (
          <section className="grid gap-4">
        ...
        {section === 'contributions' && joinRequests.length === 0 && activeMemberContributions.length === 0 && contributionHistory.length === 0 && associationCalls.length === 0 ? (
          <div className="rounded-md border border-border bg-sunken p-6 text-sm leading-6 text-secondary">{t('noMemberContributions')}</div>
        ) : null}
